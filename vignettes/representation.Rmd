...
## Intro
The `beaconstools` package contains functions for two types of representation analysis: 

* The `calc_dissimilarity()` function compares the distributions of a raster layer between a reserve and a broader study region that we are trying to represent. The closer the distributions of raster values, the lower the dissimilarity score. Values of 0.2 or less are considered 'good' representation. Examples of `calc_dissimilarity()` are shown in the 'Overview' vignette.
* In this vignette we focus on the second type of representation analysis, a target-based approach using proportional representation. This is useful when you want to ensure specific target areas of different representation classes are met in one or multiple reserves.  
...
The following functions in this vignette rely on a catchments dataset:

- `dissolve_catchments_from_table()`: converts catchment lists (e.g. from `benchmarkbuilder`) to dissolved polygons.
...
plot(vignette_catchments$geometry, add = TRUE, lwd = 2)
```
...
Target tables can be created manually as long as they contain the columns "class_value", "class_proportion" and "target_km2". The column "class_proportion" is required by `summarize_representation_results()` so should not be edited. However, if fixed instead of proportional targets are desired the "target_km2" values could be edited using for example `target_table$target_km2 <- 50`.
...
# Summarize
reserves_summary <- summarize_representation_results(
  network_evaluation_table = reserve_results,
  criteria_name = "nalc", 
...
print(reserves_summary[order(reserves_summary$nalc_gaps),])

# Join the summary results to the polygons
reserve_polygons_nalc <- reserve_polygons %>%
  left_join(reserves_summary, by = 'network')

# Map the benchmarks with the least representation gaps (i.e. 2 gaps)
plot(vignette_nalc, axes = FALSE)
plot(reserve_polygons_nalc$geometry[reserve_polygons_nalc$nalc_gaps==2], add = TRUE, border = 'cyan', lwd = 2)
```

The best benchmarks have 2 representation gaps. There are 3 of these. The existing reserves all have 6 or more gaps. Since no benchmarks pass all of our required targets we will now combine multiple benchmarks into networks to try and meet all targets.
...
# We have 30 networks that meet all the targets (i.e. they have zero gaps)
print(nrow(networks_summary[networks_summary$nalc_gaps == 0,]))

# Make a plot showing the full footprint of the best 30 networks, along with an example of one 
...
reserve_polygons <- append_reserve(
  benchmarks_sf = reserve_polygons, 
  add_reserve = vignette_existing_reserves[vignette_existing_reserves$reserve == 'Tombstone_1',],
...
# We have 2 networks that meet all but 1 of the targets
print(nrow(networks_summary[networks_summary$nalc_gaps == 1,]))

# make a plot showing the full footprint of the best 2 networks, along with an example of one of 
# the networks
network_polygons <- benchmarks_to_networks(
  benchmarks_sf = reserve_polygons, 
  network_list = networks_summary$network[networks_summary$nalc_gaps == 1])
