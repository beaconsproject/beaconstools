...
# Run builder() using the catchments, the seeds table defining the 500km2 area target for the 2527 seeds, 
...
benchmarks_tab <- builder(catchments_sf = builder_catchments_sample, 
                          seeds = seed, 
                          neighbours = nghbrs,
...
### Filter reserves
```{r, fig.width=7, fig.height=5, fig.keep = 'last', message=FALSE}
# We have 166 reserves which when combined into networks will create a large number options, 
# slowing down our analysis.
# We can filter out redundant reserves by combining them into spatially similar groups, and selecting 
# the best reserve from each group.
...
# Lower values indicate a closer fit between the NALC class proportions in the reserve and the study region.
...
# We could carry out the above workflow in a single dplyr call by un-commenting the code below.
# reserves_filtered <- reserves_poly %>%
...
# Intactness - calculate the intact area in each reserve using a vector map of intactn areas. In 
...
# Add a second dissimilarity values for Lake Edge Density.
...
reserves_filtered$led_dissimilarity <- calc_dissimilarity(reserves_filtered, fda, vignette_led, 'continuous')
...
# segments.
plot(reserves_filtered$geometry[reserves_filtered$network == "PB_359630"], lwd = 2)
...
plot(fda$geometry)
plot(upstream_polygons$geometry[upstream_polygons$network == 'PB_362239'], border = NA, col='cadetblue', add=T)
plot(reserves_filtered$geometry[reserves_filtered$network == 'PB_362239'], border='gold', add=T)
```
...
# Sum the forested area using sum_raster_values(). Note we could have fed in the vignette_nalc object and set 
# class_vals=1 and fill_zeros=FALSE to get the same result.
...
# class (nalc_dissimilarity < 0.2), select the reserve that is most intact, and has the least 
# upstream area.
...
```{r, fig.width=7, fig.height=5, message=FALSE, fig.keep='last'}
# Scenario 3: Select the network with the lowest cumulative rank across all ecological metrics.
# This approach uses sum_ranks() to find the reserve with the lowest cumulative rank score.
best_reserve_sc3 <- reserves_filtered %>%
  mutate(sum_of_ranks = sum_ranks(., columns_asc = c('nalc_dissimilarity', 'led_dissimilarity', 'upstream_km2'), columns_desc = c('dci', 'upstream_AWI', 'forested_km2', 'intact_proportion'))) %>%
...
# Generate networks
Our 28 reserves did quite well in terms of their ecological metrics and representation scores. For the purpose of an example, lets suppose we want to create a reserve network of two reserves, each at least 500km2 is size. The reserves should not be overlapping.
...
# Build our list of 220 networks using the reserve polygons.
...
# Since we have a large number of polygons to process, this will take longer than for the reserves.
...
# To reduce processing time we can filter out networks that don't meet our representation requirement 
# of dissimilarity values of <0.2. his will save processing time when calculating upstream areas.
networks <- networks %>%
  filter(nalc_dissimilarity < 0.2, led_dissimilarity < 0.2)
...
plot(fda$geometry)
plot(network_upstream_polygons$geometry[network_upstream_polygons$network == 'PB_360376__PB_363766'], border = NA, col='cadetblue', add=T)
plot(networks$geometry[networks$network == 'PB_360376__PB_363766'], col='gold', add=T)
```
...
```{r, fig.width=7, fig.height=5, message=FALSE, fig.keep='last'}
# Use sum_ranks() to select the top network.
best_networks <- networks %>%
  mutate(sum_of_ranks = sum_ranks(., columns_asc = c('nalc_dissimilarity', 'led_dissimilarity', 'upstream_km2'), columns_desc = 'upstream_AWI')) %>%
  arrange(sum_of_ranks)
